...
##### 3.在桌面新增資料夾「workspace」並在vscode中開啟資料夾
...
若要建立新文章，在blog下的終端機中輸入`hexo new filename` (filename輸入檔名)。
文章可用markdown語法撰寫。
文章撰寫完成在終端機中輸入
`hexo clean`
`hexo s` 
再重新整理`http://localhost:4000/` 顯示網頁更新。
...
回到終端機輸入`ssh -T git@github.com`
若出現`Are you sure you want to continue connecting (yes/no/[fingerprint])?`，輸入`yes`
看到`Hi 你的帳號! You've successfully authenticated...`代表成功
##### 7.設定自動部署
在blog下終端機輸入`npm install hexo-deployer-git --save`
